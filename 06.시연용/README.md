...
이 과정은 **SAM3 추론이 아니라 라벨에서 CSV·정량값·오버레이를 만드는 작업**입니다. CPU만으로 실행할 수 있습니다. Python 3.11 환경에서 실행하십시오.
...
[시연 모델 Release](https://github.com/Jiwandukong/UAV_RGB_Damage_Module/releases/tag/sam3-demo512-v1)에서 약 3.37GB 모델을 받습니다. `06.시연용/`에서 다음 명령을 실행하면 다운로드·복원·파일 검사를 자동으로 진행합니다.

```bash
python code/모델받기.py
```

[모델 저장 위치·다운로드 안내](02.시연모델/README.md)와 [추론 실행 명령](code/실행안내.md#4-실제-모델로512타일-한-장-예측)을 참고하십시오. 다운로드가 안 되면 관리자에게 모델 파일을 직접 전달받으십시오.

**실제 모델 예측은 라벨과 동일하지 않습니다.** 현재 저장된 모델 예측은 GPU로 생성했습니다. 이 시연 모델의 CPU 추론 완료 여부는 별도 검증이 필요하며, 위의 CPU 안내는 라벨 기반 산출물 생성에 대한 설명입니다.
